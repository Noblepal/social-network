...
import com.famousindiasocialnetwork.adapter.MessageAdapter;
import com.famousindiasocialnetwork.fragment.ConfirmationDialogFragment;
import com.famousindiasocialnetwork.listener.OnMessageItemClick;
import com.famousindiasocialnetwork.listener.RecordingViewInteractor;
...
import com.famousindiasocialnetwork.model.DownloadFileEvent;
...
import com.famousindiasocialnetwork.util.DownloadUtil;
...
public class MessagesActivity extends BaseChatActivity implements OnMessageItemClick, View.OnClickListener, RecordingViewInteractor, ImagePickerCallback, AudioPickerCallback, VideoPickerCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.hasExtra(EXTRA_DATA_USER)) {
            ...
            Constants.CURRENT_CHAT_ID = user.getId();
        } else {
        ...
        Glide.with(this).load(user.getImage())
                .apply(RequestOptions.bitmapTransform(new RoundedCorners(Helper.dp2px(this, 8))).placeholder(R.drawable.ic_person_gray_24dp)).into(usersImage);

        //setup chat child reference of logged in user and selected user
        chatChild = Helper.getChatChild(user.getId(), userMe.getId());
        userOrGroupId = user.getId();

        //setup recycler view
        messageAdapter = new MessageAdapter(this, dataList, userMe.getId());
    ...
    private void registerUserUpdates() {
        ...
        newMessage.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                sendMessage.setImageDrawable(ContextCompat.getDrawable(MessagesActivity.this, s.length() == 0 ? R.drawable.ic_keyboard_voice_24dp : R.drawable.ic_send_message));
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (mRecorder != null && newMessage.getText().toString().trim().isEmpty()) {
                        if (Math.abs(event.getX()) / displayWidth > 0.35f) {
                            recordingStop(false);
                            Toast.makeText(MessagesActivity.this, "Recording cancelled", Toast.LENGTH_SHORT).show();
                        }
                    }
                    break;
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.send:
                ...
                break;
            case R.id.userImage:
                startActivity(UserProfileDetailActivity.newInstance(this, user.getId(), user.getName(), user.getImage()));
                break;
...
//                break;
                ...
                ConfirmationDialogFragment confirmationDialogFragment = ConfirmationDialogFragment.newInstance("Delete messages",
                        ...
                        new View.OnClickListener() {
                            ...
                            public void onClick(View view) {
                                ...
                                getSupportActionBar().setTitle(userMe.getName());
    ...
    private void prepareMessage(String body, int attachmentType, Attachment attachment) {
        ...
        Integer userId = message.getRecipientId();
        Integer myId = message.getSenderId();
    ...
    private void newFileUploadTask(String filePath, @AttachmentTypes.AttachmentType final int attachmentType, final Attachment attachment) {
        ...
        Intent intent = new Intent(Constants.UPLOAD_AND_SEND);
    ...
    public void OnMessageLongClick(Message message, int position) {
        if (!Constants.CHAT_CAB && RealmObject.isValid(message)) {//Prepare selection if not in selection mode
            prepareToSelect();
    ...
    public static Intent newIntent(Context context, ArrayList<Message> forwardMessages, UserRealm user) {
